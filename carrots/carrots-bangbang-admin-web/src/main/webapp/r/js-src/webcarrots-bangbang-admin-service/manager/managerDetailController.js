...
angular.module('carrots_bangbang')
    .controller('managerDetailCtrl', function($state,$scope, $rootScope,commonUtil,managerService) {
        var vm = $scope.vm = {};
        vm.id = $state.params.id;

        if (vm.id) {
            managerService.getModule(vm.id).then(function(res) {
                if (res.data.code == 0) {
    ...
    vm.add = function() {
        ...
        managerService.add(vm.params).then(function(res) {
            if (res.data.code === 0) {
    ...
    vm.update = function() {
        ...
        managerService.update(id, vm.params).then(function(res) {
            if (res.data.code === 0) {
